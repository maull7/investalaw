...
@section('content')
    ...
    <section class="relative overflow-hidden rounded-[24px] bg-navy-gradient text-white p-7 sm:p-9">
        ...
        <div class="relative grid lg:grid-cols-3 gap-6 items-start">
            <div class="lg:col-span-2">
                ...
                <h2 class="mt-4 text-3xl font-bold tracking-tight">{{ $subCategory->name }}</h2>
                @if ($subCategory->description)
                    <p class="mt-3 text-white/70 leading-relaxed max-w-3xl">{{ $subCategory->description }}</p>
                @endif
            </div>
            ...
            <div class="rounded-2xl border border-white/10 bg-white/5 backdrop-blur p-5">
                ...
                <div class="mt-3 flex items-baseline gap-2">
                    <p class="text-4xl font-bold text-white">{{ $subCategory->regulations->count() }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
        <div class="lg:col-span-2 space-y-6">
            <x-card>
                <x-slot name="header">
                    <h3 class="text-lg font-bold text-[#071833]">Sub Category Information</h3>
                </x-slot>
                <dl class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        <dt class="text-[11px] font-bold uppercase tracking-wider text-[#667085]">Name</dt>
                        <dd class="mt-1.5 text-sm font-semibold text-[#071833]">{{ $subCategory->name }}</dd>
                    </div>
                    <div>
                        ...
                        <dd class="mt-1.5 text-sm font-semibold text-[#071833]">
                            {{ $subCategory->created_at->format('d F Y') }}
                        </dd>
                    </div>
                </dl>
                @if ($subCategory->description)
                    <div class="mt-6 pt-6 border-t border-[#e7eaf0]">
                        ...
                        <p class="mt-2 text-sm text-[#071833] leading-relaxed">{{ $subCategory->description }}</p>
                    </div>
                @endif
            </x-card>
                ...
                <x-slot name="header">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span
                                class="px-3 py-1 rounded-full bg-[#f6f8fb] text-xs font-bold text-[#667085]">{{ $subCategory->regulations->count() }}
                                regulasi</span>
                        </div>
                    </div>
                </x-slot>

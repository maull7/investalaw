...
<div x-data="{ parsedSubTab: 'document' }">
    ...
    <div class="flex gap-1 bg-[#f6f8fb] rounded-2xl p-1.5 ring-1 ring-[#e7eaf0] mb-4">
        ...
        <button @click="parsedSubTab = 'regulations'"
                :class="parsedSubTab === 'regulations' ? 'bg-white shadow-sm ring-1 ring-[#e7eaf0] text-[#071833]' : 'text-[#667085] hover:text-[#071833]'"
                class="flex-1 px-4 py-2.5 rounded-xl text-sm font-semibold transition">
            <span class="flex items-center justify-center gap-2">
                Regulasi Acuan
                <span class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-700">{{ count($parsedRegulations ?? []) }}</span>
            </span>
        </button>
    </div>

    {{-- Sub-tab: Document --}}
    <div x-show="parsedSubTab === 'document'" class="space-y-4">
        @if(!empty($docPages))
            {{-- Summary stats --}}
            <x-card :padding="false">
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 p-6">
                    ...
                    <div class="text-center">
                        <p class="text-2xl font-bold text-[#071833]">{{ number_format($docTotalChars ?? 0) }}</p>
                    ...
                    <div class="rounded-2xl border border-[#e7eaf0] overflow-hidden bg-white">
                        ...
                        <div x-show="expandedPage === {{ $pageData['page'] }}" x-collapse>
                            <div class="px-5 pb-5">
                                ...
                                @else
                                    <p class="text-sm text-[#667085] italic">Halaman ini tidak mengandung teks yang bisa diekstrak.</p>
                                @endif
                            </div>
                        </div>
                    </div>
        ...
        @else
            <x-card>
                <p class="text-center text-sm text-[#667085] py-8">Tidak ada data hasil parse dokumen.</p>
            </x-card>
        @endif
    </div>

    {{-- Sub-tab: Regulations --}}
    <div x-show="parsedSubTab === 'regulations'" x-cloak class="space-y-4">
        @if(!empty($parsedRegulations))
            <div x-data="{ expandedReg: null, expandedDoc: null }" class="space-y-3">
                @foreach($parsedRegulations as $index => $reg)
                    <div class="rounded-2xl border border-[#e7eaf0] overflow-hidden bg-white">
                        ...
                        <div x-show="expandedReg === {{ $index }}" x-collapse>
                            <div class="px-5 pb-5 space-y-4">
                                ...
                                @elseif(empty($reg['documents']))
                                    <div class="rounded-xl bg-rose-50 p-4 ring-1 ring-rose-200">
                                        <p class="text-sm text-rose-700">Tidak ada file regulasi yang terupload.</p>
                                    </div>
                                @endif
        ...
        @else
            <x-card>
                <div class="text-center py-8">
                    <p class="text-sm text-[#667085]">Belum ada regulasi acuan yang terhubung dengan dokumen ini.</p>
                </div>
            </x-card>
        @endif
    </div>

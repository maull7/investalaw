<aside
    class="fixed inset-y-0 left-0 z-40 w-72 transform transition-transform duration-300 ease-out lg:translate-x-0"
    :class="sidebar ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'"
>
    <div class="relative h-full overflow-hidden bg-navy-gradient text-white sidebar-scroll overflow-y-auto">
        {{-- Decorative glow --}}
        <div class="pointer-events-none absolute -top-24 -right-24 w-72 h-72 rounded-full bg-[#c99a3e]/20 blur-3xl"></div>
        <div class="pointer-events-none absolute bottom-0 -left-24 w-72 h-72 rounded-full bg-[#0b2a55]/40 blur-3xl"></div>
        ...
        <div class="relative flex flex-col h-full px-5 py-7">
            ...
            <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-2 group">
                <div class="relative">
                    ...
                    <span class="absolute -bottom-1 -right-1 w-3 h-3 rounded-full bg-emerald-400 ring-2 ring-[#071b3a]"></span>
                </div>
                <div>
                    <p class="text-base font-bold tracking-tight text-white">{{ config('app.name', 'InvestaLaw') }}</p>
                    <p class="text-[10.5px] font-medium tracking-[0.18em] uppercase text-[#c99a3e]">Legal · Strategic</p>
                </div>
            </a>
            ...
            <nav class="mt-7 flex-1">
                    ...
                    <li>
                        <a href="{{ route('dashboard') }}" class="nav-item {{ request()->routeIs('dashboard') ? 'is-active' : '' }}">
                            <svg class="nav-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M3 13.5 12 4l9 9.5M5 12v8a1 1 0 0 0 1 1h4v-6h4v6h4a1 1 0 0 0 1-1v-8"/>
                            </svg>
                            <span>Dashboard</span>
                        </a>
                    </li>
                ...
                <p class="px-3 mt-7 mb-2 text-[10.5px] font-semibold tracking-[0.18em] uppercase text-white/45">Master Data</p>
                <ul class="space-y-1.5">
                    <li>
                        <a href="{{ route('regulation-categories.index') }}" class="nav-item {{ request()->routeIs('regulation-categories.*') ? 'is-active' : '' }}">
                            <svg class="nav-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.6">
                ...
                <ul class="space-y-1.5">
                    <li>
                        <a href="{{ route('review-documents.index') }}" class="nav-item {{ request()->routeIs('review-documents.*') ? 'is-active' : '' }}">
                            <svg class="nav-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.6">
                    ...
                    <li>
                        <a href="{{ route('reviews.index') }}" class="nav-item {{ request()->routeIs('reviews.*') || request()->routeIs('reports.*') ? 'is-active' : '' }}">
                            <svg class="nav-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.6">
            ...
            {{-- Footer --}}
            <div class="mt-6 rounded-2xl border border-[#c99a3e]/25 bg-gradient-to-br from-[#c99a3e]/15 to-transparent p-4">
                <div class="flex items-start gap-3">
                    <div class="w-9 h-9 rounded-xl bg-[#c99a3e] flex items-center justify-center shrink-0">
                        <svg class="w-4.5 h-4.5 text-[#071b3a]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M12 2 4 5v6c0 5 3.5 9 8 11 4.5-2 8-6 8-11V5l-8-3Z"/>
                        </svg>
                    </div>
                    <div class="min-w-0">
                        <p class="text-sm font-semibold text-white">Need Assistance?</p>
                        <p class="text-[11px] text-white/60 leading-relaxed mt-0.5">Reach our compliance desk for any regulatory inquiry.</p>
                    </div>
                </div>
                <a href="mailto:support@investalaw.test" class="mt-3 inline-flex items-center justify-center gap-2 w-full text-xs font-semibold text-[#071b3a] bg-gradient-to-r from-[#c99a3e] to-[#e6c06a] rounded-xl py-2.5 hover:brightness-110 transition">
                    Contact Compliance

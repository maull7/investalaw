...
@section('title', 'Viewer — ' . $regulation->regulation_number)
@section('header', 'PDF Viewer')

@section('content')
    <div x-data="pdfViewer('{{ route('regulations.file-raw', $regulation, false) }}', true)" @keydown.arrow-left.window="prevPage()" @keydown.arrow-right.window="nextPage()"
        ...
        <div class="flex items-center justify-between gap-3 px-4 sm:px-5 py-3 border-b border-[#e7eaf0] bg-white">
            <div class="flex items-center gap-3 min-w-0">
                <a href="{{ route('regulations.show', $regulation, false) }}"
                ...
                <div class="min-w-0">
                    <p class="text-sm font-bold text-[#071833] truncate">{{ $regulation->regulation_number }} —
                        {{ $regulation->title }}</p>
                    <p class="text-[10px] text-[#667085]">{{ $regulation->year }}</p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-1.5 shrink-0">
                ...
                <div class="flex items-center gap-1 rounded-xl bg-[#f6f8fb] p-1">
                    <button type="button" @click="showingScroll = false"
                        :class="!showingScroll ? 'bg-white shadow-sm text-[#071833]' : 'text-[#667085] hover:text-[#071833]'"
                        class="px-3 py-1.5 text-xs font-bold rounded-lg transition">Presentasi</button>
                    <button type="button" @click="toggleScroll()"
                        :class="showingScroll ? 'bg-white shadow-sm text-[#071833]' : 'text-[#667085] hover:text-[#071833]'"
                        class="px-3 py-1.5 text-xs font-bold rounded-lg transition">Scroll</button>
                </div>
                ...
                <div class="flex items-center gap-1.5" x-show="!showingScroll">
                    <button type="button" @click="prevPage()" :disabled="currentPage <= 1"
                        class="p-2 rounded-xl text-[#667085] hover:bg-[#f6f8fb] hover:text-[#071833] transition disabled:opacity-40 disabled:pointer-events-none">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5" />
                        </svg>
                    </button>
                    <span class="text-xs font-semibold text-[#071833] whitespace-nowrap"
                ...
                <div class="flex items-center gap-1">
                    <button type="button" @click="zoomOut()"
                        class="p-2 rounded-xl text-[#667085] hover:bg-[#f6f8fb] hover:text-[#071833] transition">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607ZM13.5 7.5h-6" />
                        </svg>
                    </button>
                    <span class="text-xs font-semibold text-[#667085] w-10 text-center"
                ...
                {{-- <a href="{{ route('regulations.file-raw', $regulation, false) }}" download
        ...
        <div x-ref="slideWrapper" x-show="loaded && !showingScroll" class="flex-1 overflow-hidden bg-[#f6f8fb]"
            @mousedown="dragStart($event)" @mouseup="dragEnd($event)" @touchstart.passive="dragStart($event)"
            @touchend.passive="dragEnd($event)">

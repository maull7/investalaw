        ...
        <form method="GET" action="{{ route('regulations.index') }}" class="space-y-4">
            ...
            <div class="border-t border-[#e7eaf0] pt-4">
                ...
                <input type="text" name="search_content" id="search_content"
                    ...
                    placeholder="Cari kata dalam isi dokumen regulasi...">
            </div>
        </form>
    </x-card>
                        ...
                        @foreach ($regulations as $reg)
                            <tr>
                                ...
                                <td>
                                    <div>
                                        ...
                                        @if (!empty($filters['search_content']) && $reg->parsed_text)
                                            @php
                                                $snippet = app(
                                                    \App\Repositories\RegulationRepository::class,
                                                )->buildSnippet($reg->parsed_text, $filters['search_content'], 250);
                                            @endphp
                                            @if ($snippet)
                                                <div class="mt-2 p-3 bg-yellow-50 border-l-4 border-yellow-400 rounded">
                                                    <div class="flex items-start gap-2">
                                                        ...
                                                        <div class="text-xs text-[#854d0e] leading-relaxed">
                                                            {!! preg_replace(
                                                                '/(' . preg_quote($filters['search_content'], '/') . ')/iu',
                                ...
                                <td class="text-center">
                                    ...
                                    @if ($docCount > 0)
                                        <button type="button"
                                            @click="docModal = {{ $reg->id }}; docs = {{ Js::from($reg->documents->map(fn($d) => ['id' => $d->id, 'name' => $d->name, 'type' => $d->document_type])) }}"

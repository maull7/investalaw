...
@section('title', 'Edit Type Prompt')
@section('header', 'Edit Type Prompt')
            ...
            <x-card>
                <x-slot name="header">
                    <div>
                        ...
                        <h3 class="mt-1 text-xl font-bold text-[#071833]">Edit Type Prompt</h3>
                        <p class="text-sm text-[#667085] mt-1">Ubah tipe {{ $typePrompt->name }}.</p>
                    </div>
                </x-slot>

                <form method="POST" action="{{ route('type-prompts.update', $typePrompt) }}" class="space-y-6">
                    @csrf
                    @method('PUT')

                    <div>
                        <label for="name" class="block text-sm font-semibold text-[#071833] mb-2">Name <span class="text-[#c99a3e]">*</span></label>
                        <input type="text" name="name" id="name" value="{{ old('name', $typePrompt->name) }}" required class="input-premium">
                        @error('name')
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3" class="input-premium input-textarea w-full">{{ old('description', $typePrompt->description) }}</textarea>
                        @error('description')
                            <p class="mt-1.5 text-xs font-medium text-rose-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center gap-3">
                        <input type="checkbox" name="is_active" id="is_active" value="1"
                            {{ old('is_active', $typePrompt->is_active) ? 'checked' : '' }}

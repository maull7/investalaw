...
@section('content')
                ...
                <div x-ref="messages" class="max-h-[28rem] overflow-y-auto space-y-3">
                    @forelse($session->messages as $msg)
                        @if ($msg->role === 'user')
                            <div class="flex items-start justify-end gap-2.5">
                                <div class="max-w-[80%] rounded-2xl rounded-tr-md bg-navy-gradient px-4 py-3 text-sm leading-6 text-white shadow-sm">
                                    <div class="whitespace-pre-wrap break-words">{{ $msg->content }}</div>
                                </div>
                                <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[#071b3a] to-[#0b2a55] text-xs font-bold text-white ring-1 ring-[#c99a3e]/40">
                                    {{ strtoupper(mb_substr(auth()->user()->name, 0, 1)) }}
                                </div>
                            </div>
                        @else
                            <div class="flex items-start gap-2.5">
                                <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[#c99a3e] to-[#e6c06a] text-xs font-bold text-[#071b3a]">V</div>
                                <div class="max-w-[85%] min-w-0 rounded-2xl rounded-tl-md bg-[#f6f8fb] px-4 py-3 text-sm leading-6 text-[#071833] shadow-sm ring-1 ring-[#e7eaf0]">
                                    <div class="whitespace-pre-wrap break-words">{{ $msg->content }}</div>
                                </div>
                            </div>
                        @endif
                    @empty
                        <div x-ref="empty" class="text-center py-10">
                            <div class="mx-auto w-12 h-12 rounded-2xl bg-[#f6f8fb] flex items-center justify-center text-[#c99a3e]">
                    ...
                    <form @submit.prevent="send()" class="flex items-center gap-2">
                        <label class="relative flex-1">
                            <textarea x-model="question" :disabled="sending" rows="2" maxlength="4000"
                                class="input-premium resize-none" placeholder="Tanya Kak Vesta tentang regulasi…"></textarea>
                        </label>
                        <button type="submit" :disabled="sending"
                            ...
                            <svg x-show="sending" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <x-slot name="header">
                    <div class="flex items-center justify-between">
                        ...
                        <button type="button" @click="$dispatch('open-modal-add-regulations')"
                            class="text-xs font-semibold text-[#c99a3e] hover:text-[#8c6a25] transition" @disabled($session->regulations->count() >= 10)>
                            + Tambah
                        </button>
                    </div>
                </x-slot>
                <div class="space-y-2 max-h-[26rem] overflow-y-auto">
                    @forelse($session->regulations as $reg)
                        <div class="flex items-start gap-2.5 p-3 rounded-xl bg-[#f6f8fb] ring-1 ring-[#e7eaf0]">
                            <span class="shrink-0 w-6 h-6 rounded-lg bg-[#c99a3e]/10 flex items-center justify-center text-[10px] font-bold text-[#8c6a25]">{{ $loop->index + 1 }}</span>
                            <div class="min-w-0">
    ...
    <x-modal name="add-regulations" max-width="2xl" title="Tambah Regulasi">
        <form method="POST" action="{{ route('consultations.regulations.add', $session) }}">
            @csrf
            @include('review-documents._regulation-picker', ['selectedIds' => $selectedIds, 'categories' => $categories])
            <div class="mt-4 flex items-center justify-between">
                <p class="text-xs text-[#667085]">Maksimal 10 regulasi per sesi (saat ini {{ $session->regulations->count() }}).</p>
        ...
        return {
            ...
            appendBubble(role, text, typing = false) {
                ...
                bubble.className = 'min-w-0 max-w-[80%] rounded-2xl '
                    + (role === 'user' ? 'rounded-tr-md bg-navy-gradient text-white' : 'rounded-tl-md bg-[#f6f8fb] ring-1 ring-[#e7eaf0] text-[#071833]')
